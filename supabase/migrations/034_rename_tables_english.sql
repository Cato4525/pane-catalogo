...
-- Renombrar funciones/triggers asociados
...
-- =====================================================
-- 3. NOTA: clientes se mantiene como está porque:
--    - La tabla clients ya existe en inglés
--    - clientes tiene FK references desde pos_orders y direct_sales
--    - Para renombrarla se necesitaría migrar datos y FKs
--    Se recomienda hacerlo en una migración separada
-- =====================================================
...
ORDER BY table_name;
, escript corregido, ALTER TRIGGER trigger_updated_at ON public.pos_orders 
RENAME TO trigger_updated_at_pos_orders;
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
    ...
    AND trigger_name = 'trigger_updated_at'
  ) THEN
    ALTER TRIGGER trigger_updated_at 
...
BEGIN
  IF EXISTS (
    ...
    FROM information_schema.triggers
    WHERE event_object_table = 'pos_orders'
    AND trigger_name = 'trigger_codigo_pedido'
  ) THEN
    ALTER TRIGGER trigger_codigo_pedido 
    ON public.pos_orders 
    RENAME TO trigger_codigo_pos_order;
  END IF;
END $$;
SELECT trigger_name
FROM information_schema.triggers
